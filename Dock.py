...
class Dock_Harbor_Manager:
    ...
    def register_ship_leave(self, ship, time):
        mytime = self.total_register[ship][0]
    ...
    def register_harbor_leave(self, ship, time):
        mytime = self.harbor_register[ship][0]
        self.harbor_register[ship] = (mytime, time)

    def any_harbor_free(self):
        pass
    
    def any_ship_in_harbor(self):
        return self.ship_harbor_waiting.count > 0
    
    def any_ship_in_port(self):
        return self.ship_port_waiting.count > 0
    ...
    def ship_port_waiting_count(self):
        return self.ship_port_waiting.count
    
    def ship_harbor_waiting_count(self):
        return self.ship_harbor_waiting.count
    ...
    def enter_ship_harbor(self, ship):
        i = 0
        for x in len(self.ship_loading):
            if x is None:
